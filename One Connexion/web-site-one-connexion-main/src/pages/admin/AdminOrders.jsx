        ...
        actions={
          <>
            {view === "all" && (
              <button
                onClick={exportCsv}
                className="group flex items-center gap-3 rounded-2xl border border-noir/5 bg-white px-6 py-4 text-xs font-bold uppercase tracking-[0.2em] text-noir shadow-sm hover:bg-noir hover:text-white transition-all"
              >
            ...
            <div className="flex bg-cream p-1.5 rounded-2xl border border-noir/5 gap-1 shadow-inner">
              {TABS.map(tab => (
                <button
                  ...
                  className={`flex items-center gap-3 px-6 py-3.5 rounded-xl text-xs font-bold uppercase tracking-widest transition-all duration-300 ${view === tab.id ? 'bg-noir text-white shadow-xl' : 'text-noir/40 hover:text-noir'}`}
                >
                  <tab.icon size={14} strokeWidth={view === tab.id ? 2.5 : 1.5} />
                  <span>{tab.label}</span>
                  <div className="flex items-center gap-2">
                    {tab.badge > 0 && (
                      <span className="h-5 min-w-[20px] rounded-full bg-[#ed5518] text-white text-[10px] font-bold grid place-items-center shadow-[0_0_15px_rgba(237,85,24,0.3)]">
                        {tab.badge}
                      </span>
                    )}
                    <span className={`h-5 min-w-[20px] rounded-full text-[10px] font-bold grid place-items-center ${view === tab.id ? 'bg-white/20 text-white' : 'bg-noir/10 text-noir/40'}`}>
                      {tab.count}
                    </span>
                  </div>
                </button>
        ...
        >
          ...
          <div className="flex items-center gap-8 relative z-10">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-red-600/60 uppercase tracking-[0.4em]">
                {claimCount} Litige(s) en attente de résolution prioritaire.
              </p>
            </div>
          </div>
          <button className="mt-8 md:mt-0 px-10 py-5 rounded-2xl bg-noir text-white text-xs font-bold uppercase tracking-[0.3em] group-hover:bg-[#ed5518] transition-all shadow-xl">
            Ouvrir la médiation
          </button>
